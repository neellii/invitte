...
<main class="main">
  <div class="create">
    <h1 class="create-title">Заполните данные о вашем мероприятии</h1>

    <form class="form" action="/templates" method="post" enctype="multipart/form-data">
      ...
      <div class="q-group">
        ...
        <input id="party_date" name="party_date" placeholder="31 Июля 2024" value="<?= old('party_date') ?>">
      ...
      <div class="add-to-div">
        <div class="programm-inputs">
          <div>
            <label for="programm_name1" class="form-label" data-number="1">название</label>
      ...
      <div class="q-group">
        ...
        <input type="text" id="hashtag" placeholder="#dobrovywedding" name="hashtag" value="<?= old('hashtag') ?>">
        <?= isset($validation) ? $validation->listErrors('hashtag') : '' ?>
      </div>

      <div class="q-group">
        <label for="date_timer">Дата для таймера в формате: гггг-мм-дд</label>
        <input type="text" id="date_timer" placeholder="2024-06-12" name="date_timer" value="<?= old('date_timer') ?>">
        <?= isset($validation) ? $validation->listErrors('date_timer') : '' ?>
      </div>

      <div class="q-group">
        <label for="note">Примечание</label>
        <input type="text" id="note" placeholder="пожалуйста, приносите вместо цветов - воздушные шары" name="note" value="<?= old('note') ?>">
        <?= isset($validation) ? $validation->listErrors('note') : '' ?>
      </div>

      <div class="q-group">
        <label for="contacts">Контакт для гостей - ссылка для связи</label>
        <input type="text" id="contacts" placeholder="свадебный организатор Юлия" name="contacts" value="<?= old('contacts') ?>">
        <input type="url" id="contacts_url" placeholder="https://t.me/yuliyawedding" name="contacts_url" value="<?= old('contacts_url') ?>" style="margin-top: 8px">
      ...
      <div class="ques rsvp-group">
        ...
        <div class="q-group" data-ques="1">
          ...
          <input type="text" id="ques" name="rsvp_ques[1]" value="<?= old('rsvp_ques[1]') ?>" placeholder="Нуждаетесь ли вы в парковочном месте?">
        </div>
      </div>
      ...
      <div class="q-group">
        ...
        <input type="text" id="og_title" name="og_title" value="<?= old('og_title') ?>" placeholder="Кирилл и Ульяна">
        <?= isset($validation) ? $validation->listErrors('og_title') : '' ?>
      </div>

      <div class="q-group">
        <label for="og_desc">Введите описание вашего приглашения</label>
        <input type="text" id="og_desc" name="og_desc" value="<?= old('og_desc') ?>" placeholder="Приглашаем Вас на нашу свадьбу!">
      ...
      <div class="q-group">
        <label for="slug">Введите желаемый текст для ссылки <span>*</span><br> разрешено использовать только латиницу и знак дефиса (-)</label>
        <input id="slug" name="slug" placeholder="alexey-elena-dobrovy" value="<?= old('slug') ?>">
        <?= isset($validation) ? $validation->listErrors('slug') : '' ?>
      </div>

      <input type="hidden" value="<?= $event_id ?? $_POST['event_id'] ?>" name="event_id">
      ...
      <p class="dop-title">Проверьте правильность заполнения данных. Если вы обновляли страницу или получали ошибки - некоторые данные могли быть утеряны.</p>
...
<script type="text/javascript">
  ...
  addToColorBtn.addEventListener("click", (e) => {
    ...
    addToDivColor.insertAdjacentHTML('beforeend', `<div class="add-group">
              ...
              <input type="text" id="hex_color${maxColorNumber + 1}" placeholder="#84b5ff" name="hex_color[${maxColorNumber + 1}]" value=""></div>`);
  })
  ...
  addToQuesBtn.addEventListener("click", (e) => {
    ...
    addToDivQues.insertAdjacentHTML('beforeend', `<div class="add-group">
        ...
        <input type="text" id="ques${maxQuesNumber + 1}" name="rsvp_ques[${maxQuesNumber + 1}]" value="" placeholder="Какой вы дадите совет молодоженам?"></div>
`)
  })
